...
export function LoginPage() {
  const { user, signIn, verifyOtp, pendingChallenge } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const emailRef = useRef<HTMLInputElement>(null);
  const [email, setEmail] = useState('admin@mercury.local');
  const [password, setPassword] = useState('MercuryDemo@2026');
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true);
    try {
      if(pendingChallenge){await verifyOtp(otp);const target=(location.state as {from?:string}|null)?.from??'/';navigate(target,{replace:true});}
        ...
        <Box sx={{ width: '100%', maxWidth: 430 }}>
          ...
          <Card sx={{ boxShadow: '0 18px 50px rgba(32,36,31,.08)' }}><CardContent sx={{ p: { xs: 3, sm: 4 }, '&:last-child': { pb: { xs: 3, sm: 4 } } }}>
            <form onSubmit={submit} noValidate>
              ...
              {!pendingChallenge&&<><TextField inputRef={emailRef} fullWidth label="Work email" type="email" autoComplete="username" value={email} onChange={(e) => { setEmail(e.target.value); setError(''); }} sx={{ mb: 2 }} />
              ...
              {pendingChallenge&&<><Alert severity="info" sx={{mb:2}}>Enter the code sent to {pendingChallenge.maskedMobile}. {pendingChallenge.developmentOtp&&`Local code: ${pendingChallenge.developmentOtp}`}</Alert><TextField fullWidth label="6-digit code" inputMode="numeric" autoComplete="one-time-code" value={otp} onChange={(event)=>setOtp(event.target.value.replace(/\D/g,'').slice(0,6))}/></>}
              <Button type="submit" variant="contained" fullWidth disabled={busy||(pendingChallenge?otp.length!==6:false)} aria-busy={busy} sx={{ mt: 3, minHeight: 46 }}>{busy ? <CircularProgress size={21} color="inherit" aria-label="Signing in" /> : pendingChallenge?'Verify and sign in':'Continue'}</Button>
            </form>
            <Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 2.5, textAlign: 'center' }}>Local demo: admin@mercury.local · MercuryDemo@2026</Typography>
          </CardContent></Card>
        </Box>
      </Box>
